  ...
  <div v-else class="task-carousel">
    ...
    <div class="task-navigation">
      <button 
        ...
        @click="currentTaskIndex = Math.max(0, currentTaskIndex - 1)"
      >
        ← Previous
      </button>
      ...
      <button 
        ...
        @click="currentTaskIndex = Math.min(tasks.length - 1, currentTaskIndex + 1)"
      >
        Next →
      </button>
    </div>
    ...
    <!-- 顯示當前任務細節 -->
    <div v-if="tasks[currentTaskIndex]" class="task-detail">
      <div class="info-row">
        <strong>ID:</strong> {{ tasks[currentTaskIndex].id || 'N/A' }}
      </div>
      <div class="info-row">
    ...
    <div v-else class="no-task">
      No task to display (index error)
    </div>
  </div>
</div>
